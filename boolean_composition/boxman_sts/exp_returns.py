...
goals = []
if os.path.exists('boxman_sts/goals.h5'):
    goals = dd.io.load('boxman_sts/goals.h5')

env = CollectEnv()
dqn_blue = load('boxman_sts/models/blue/model.dqn', env, map_location='mps')
dqn_square = load('boxman_sts/models/crate/model.dqn', env, map_location='mps')
...
def evaluate_onoff(name='or', max_trajectory = 20):  
    if name == 'B.S':
        dqn = dqn_and
...
for i in range(num_runs):
    ...
    dd.io.save('boxman_sts/data/exp_returns_0.h5', data0)
    dd.io.save('boxman_sts/data/exp_returns_1.h5', data1)
