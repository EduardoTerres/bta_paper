...
class TaskPrimitive(gym.Env):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        assert hasattr(env, "predicates"), (
            "Your environment does not contain the list of all atomic predicates: predicates"
        )
        assert hasattr(env, "get_predicates"), (
            "Your environment doas not contain the get_predicates method: get_predicates()-->true_propositions"
        )
        if not hasattr(env, "constraints"):
            warnings.warn(
                "Your environment does not contain the list of all constraints. We will set constraints=predicates by default."
            )
            env.constraints = env.predicates

        self.render_mode = render_mode
        self.environment, self.primitive, self.rmin, self.rmax, self.use_env_rewards = (
            env,
            primitive,
            rmin,
            rmax,
        ...
        (self.np_random, self.seed), self.max_episode_steps = (
            seeding.np_random(seed),
            max_episode_steps,
        )
        self.rewards, self.successes = (
            deque(maxlen=max_episode_steps * 100),
            deque(maxlen=max_episode_steps * 100),
        )
        self.wvf, self.sb3 = wvf or sb3, sb3

        self.constraints_mask = np.array(
            [1 * (e in env.constraints) for e in env.predicates], dtype=np.uint8
        )
        self.predicates, self.get_predicates, self.constraints = (
            env.predicates,
            env.get_predicates,
            env.constraints,
        )
        self.goal_space = gym.spaces.Box(
            low=0, high=1, shape=(2 * len(env.predicates),), dtype=np.uint8
        )
        self.goals = {
            self.goal_space.low.tobytes(): self.goal_space.low.copy()
        }  # Goals buffer
        self.proposition_space = gym.spaces.Box(
            low=0, high=1, shape=(len(env.predicates),), dtype=np.uint8
        )

        # Adding violated constraints to env states. 'achieved_goal' and 'desired_goal' are for the GoalEnv interface
        self.observation_space_dict = {
            "env_state": env.observation_space,
            "violated_constraints": self.proposition_space,
        }
        if self.wvf:
            self.observation_space_dict.update({"desired_goal": self.goal_space})
        if self.sb3:
            self.observation_space_dict.update({"achieved_goal": self.goal_space})
    ...
    def split_action(self, action=None):
        if self.sb3:
            ...
            return action[:-1], action[-1] >= (
                self.action_space.low.min() + self.action_space.low.max()
            ) / 2
        else:
            return (
                action % self.environment.action_space.n,
                action >= self.environment.action_space.n,
            )

    def compute_reward(
        self, achieved_goal, desired_goal=None, env_reward=[0], done=False
    ):
        ...
        reward = (
            env_reward if self.use_env_rewards else np.zeros(achieved_goal.shape[0])
        )
        # Terminal rewards
        reward = (1 - done) * reward + done * (
            successes * self.rmax + (1 - successes) * self.rmin
        )
        # Extended rewards
        if self.wvf:
    ...
    def reset(self, seed=None, **kwargs):
        ...
        self.violated_constraints = (
            self.proposition_space.sample() & self.constraints_mask
        ) * self.np_random.choice([0, 1])

        self.achieved_goal = np.concatenate(
            [self.true_propositions, self.violated_constraints]
        )
        ag_key = self.achieved_goal.tobytes()
        if ag_key not in self.goals:
            self.goals[ag_key] = self.achieved_goal
        self.desired_goal = self.np_random.choice(list(self.goals.values()))
        self.desired_goal_key = self.desired_goal.tobytes()

        info = {
            "true_propositions": self.true_propositions,
            "violated_constraints": self.violated_constraints,
        }
        info.update(env_info)

        state = {
            "env_state": self.env_state.copy(),
            "violated_constraints": self.violated_constraints.copy(),
        }
        if self.wvf:
            state.update({"desired_goal": self.desired_goal.copy()})
        if self.sb3:
            state.update({"achieved_goal": self.achieved_goal.copy()})
        return state, info

    def step(self, action):
        env_action, done_action = self.split_action(action)
        self.env_state, env_reward, env_done, env_truncated, env_info = (
            self.environment.step(env_action)
        )

        true_propositions, violated_constraints = (
            self.true_propositions,
            self.violated_constraints.copy(),
        )
        self.true_propositions = self.environment.get_predicates()
        self.violated_constraints |= (
            true_propositions ^ self.true_propositions
        ...
        self.achieved_goal = np.concatenate(
            [self.true_propositions, violated_constraints]
        )
        ag_key = self.achieved_goal.tobytes()
        if ag_key not in self.goals:
            self.goals[ag_key] = self.achieved_goal
        ...
        if self.sb3:
            done_action = (self.achieved_goal == self.desired_goal).min()
        ...
        reward = self.compute_reward(
            self.achieved_goal.reshape(1, -1),
            self.desired_goal.reshape(1, -1),
            env_reward,
            done,
        )[0]
        self.rewards.append(reward)
        self.successes.append(reward >= self.rmax)
...
class SkillPrimitive(BaseAgent):
    def __init__(self, primitive, wvfs, goals, predicates):
        self.predicates, self.primitive, self.wvfs, self.goals, self.goal = (
            predicates,
            primitive,
            wvfs,
            goals,
        ...
        assert ("0" in wvfs) and ("1" in wvfs), (
            "wvfs should contain the min wvf ('0') and max wvf ('1')."
        )
    ...
    def get_action_value(self, states, desired_goal=None, vectorised=False):
        ...
        if type(desired_goal) != type(None):
            ...
            ### Many states
            n_goals, states["desired_goal"] = (
                len(goals),
                np.concatenate([goals] * shape[0], axis=0),
            )
...
class ComposeSkillPrimitive(SkillPrimitive):
    def __init__(self, skill_primitives, compose="or"):
        ...
        super().__init__(
            compose,
            skill_primitives[0].wvfs,
            skill_primitives[0].goals,
            skill_primitives[0].predicates,
        )
    ...
    def get_action_goal_values(self, states):
        ...
        if self.compose == "or":
            ...
            actions, values = (
                all_actions[mask, np.arange(all_values.shape[1])],
                all_values[mask, np.arange(all_values.shape[1])],
            )
            ...
            max_actions, max_values = (
                self.skill_primitives[0].wvfs["1"].get_action_value(states)
            )
...
def exp_wvf(exp, skill_primitives):
    ...
    exp = sympify(exp)
    if not exp:
        return skill_primitives["0"]

    def convert(exp):
        if type(exp) in [boolalg.Or, boolalg.And]:
            compound = convert(exp.args[0])
        if type(exp) == Symbol:
            compound = skill_primitives[str(exp)]
        elif type(exp) == boolalg.Or:
            for sub in exp.args[1:]:
                compound = ComposeSkillPrimitive([compound, convert(sub)], compose="or")
        elif type(exp) == boolalg.And:
            for sub in exp.args[1:]:
                compound = ComposeSkillPrimitive(
                    [compound, convert(sub)], compose="and"
                )
        elif type(exp) == boolalg.Not:
            compound = ComposeSkillPrimitive([convert(exp.args[0])], compose="not")
        elif type(exp) in [bool, boolalg.BooleanTrue, boolalg.BooleanFalse]:
            compound = skill_primitives["1"] if exp else skill_primitives["0"]
        else:
            assert False, (
                "There is an unknown symbol in the expression: "
                + str(exp)
                + str(type(exp))
            )
        return compound
...
class SkillMachine:
    def __init__(
        ...
        self.skill_primitives = {
            primitive: SkillPrimitive(
                ...
                skill_primitives,
                primitive_env.goals,
                primitive_env.predicates,
            )
            for primitive in skill_primitives
        }
        for proposition in primitive_env.predicates:
            self.skill_primitives[("p_" + proposition)] = SkillPrimitive(
                "p_" + proposition,
                skill_primitives,
                primitive_env.goals,
                primitive_env.predicates,
            )
        for constraint in primitive_env.constraints:
            self.skill_primitives[("c_" + constraint)] = SkillPrimitive(
                "c_" + constraint,
                skill_primitives,
                primitive_env.goals,
                primitive_env.predicates,
    ...
    def build(self, rm):
        ...
        for u in rm.delta_u:
            ...
            exp_predicates = sympify(
                exp_predicates.replace("!", "~")
                .replace("1", "true")
                .replace("0", "false")
            ).simplify()
            exp_constraints = sympify(
                exp_constraints.replace("!", "~")
                .replace("1", "true")
                .replace("0", "false")
            ).simplify()
            if type(exp_predicates) != bool:
                exp_predicates = exp_predicates.subs(
                    {symbol: f"p_{symbol}" for symbol in exp_predicates.free_symbols}
                )
            if type(exp_constraints) != bool:
                ...
                symbols = {
                    symbol: (
                        f"c_{symbol}"
                        if f"c_{symbol}" in self.skill_primitives
                        else "false"
                    )
                    for symbol in exp_constraints.free_symbols
                }
            ...
            for exp, u_next in rm.delta_u[u].items():
                self.sm_str += "({},{},'{}',WorldValueFunction({}))\n".format(
                    u, rm.delta_u[u][exp], exp, self.delta_q[u]
                )
        self.sm_str = (
    ...
    def get_action_value(self, states):
        states = {
            "env_state": states["env_state"],
            "violated_constraints": np.repeat(
                np.expand_dims(self.violated_constraints, 0),
                repeats=len(states["env_state"]),
                axis=0,
            ),
            "achieved_goal": np.repeat(
                np.expand_dims(self.goal_space.low, 0),
                repeats=len(states["env_state"]),
                axis=0,
            ),
        }
        action, value = self.wvf.get_action_value(
            states, desired_goal=self.goal, vectorised=self.vectorised
        )
...
def evaluate(
    ...
    while episode < episodes:
        ...
        while True:
            ...
            step += 1
            steps += 1
            if done or truncated or step >= eval_steps:
                break
